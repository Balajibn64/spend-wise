...
const BorrowLendRow = memo(function BorrowLendRow({
  entry,
  onSettle,
  onEdit,
  onDelete,
  ...
  entry: BorrowLend;
  onSettle: (entry: BorrowLend) => void;
  onEdit: (entry: BorrowLend) => void;
  onDelete: (id: string) => void;
}) {
  return (
    <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between hover:bg-violet-50/50 dark:hover:bg-violet-500/5 transition-colors">
      {/* Left: Person info + badges */}
      <div className="flex items-start gap-3 flex-1 min-w-0">
        {/* Avatar */}
        <div
          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-semibold text-white ${
            entry.type === "LENT" ? "bg-emerald-500" : "bg-red-500"
          }`}
        >
            ...
            <div className="mt-1.5 h-1.5 w-full max-w-xs overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
              <div
                className={`h-full rounded-full transition-all ${
                  entry.status === "SETTLED" ? "bg-emerald-500" : "bg-blue-500"
                }`}
                style={{
                  width: `${Math.min((entry.settledAmount / entry.amount) * 100, 100)}%`,
                }}
              />
            </div>
          )}
...
export default function BorrowLendPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "Failed to create entry")),
  });
  ...
  const settleMutation = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "Failed to record settlement")),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => toast.error(getApiErrorMessage(error, "Failed to delete entry")),
  });
